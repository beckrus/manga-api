...
async def test_authors_api_flow_auth(name: str, ac_auth: AsyncClient):
    """
    ...
    """
    res_get = await ac_auth.get("/authors")
    ...
    res_get_by_id = await ac_auth.get(f"/authors/{author_id}")
    assert res_get_by_id.status_code == 200
    data_get_by_id = res_get_by_id.json()
    assert data_get_by_id == data_patch
    res_delete = await ac_auth.delete(f"/authors/{author_id}")
    assert res_delete.status_code == 204
    res_get_after_delete = await ac_auth.get("/authors")
...
async def test_authors_api_flow_wo_auth(name: str, ac: AsyncClient):
    """
    Test the full CRUD flow for the authors API.
    ...
    res_delete = await ac.delete(f"/authors/{get_data[0]['id']}")
