...
async def test_manga_flow(
    ...
    res_get_by_id = await ac.get(f"/manga/{manga_id}")
    assert res_get_by_id.status_code == 200
    data_get_by_id = res_get_by_id.json()
    assert data_get_by_id == data_patch
    res_delete = await ac.delete(f"/manga/{manga_id}")
    assert res_delete.status_code == 204
    res_get_after_delete = await ac.get("/manga?page=1&per_page=100&sort=ASC")
